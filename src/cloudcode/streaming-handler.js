...
export async function* sendMessageStream(anthropicRequest, accountManager, fallbackEnabled = false) {
    ...
    function rewriteToFallback(reason) {
        ...
        for (const fallbackModel of chain) {
            ...
            if (avail.length > 0) {
                logger.warn(`[CloudCode] Pool exhausted for ${currentModel} (${reason}). Rewriting to ${fallbackModel} (streaming)`);
    ...
    outerLoop: while (true) {
        ...
        try {
            ...
            logger.debug(`[CloudCode] Starting stream for model: ${currentModel}`);
            ...
            while (endpointIndex < ANTIGRAVITY_ENDPOINT_FALLBACKS.length) {
                ...
                try {
                    ...
                    const response = await throttledFetch(url, {
                        method: 'POST',
                        headers: buildHeaders(token, currentModel, 'text/event-stream', payload.request.sessionId),
                        ...
                        try {
                            ...
                            if (!currentResponse.ok) {
                                ...
                                if (currentResponse.status === 429) {
                                    ...
                                    accountManager.markRateLimited(account.email, resetMs, currentModel);
